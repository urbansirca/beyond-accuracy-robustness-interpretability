...
from . import criss_cross_transformer
...
class CBraMod(nn.Module):
    def __init__(self, in_dim=200, out_dim=200, d_model=200, dim_feedforward=800, seq_len=30, n_layer=12,
        ...
        encoder_layer = criss_cross_transformer.TransformerEncoderLayer(
            d_model=d_model, nhead=nhead, dim_feedforward=dim_feedforward, batch_first=True, norm_first=True,
            activation=F.gelu
        )
        self.encoder = criss_cross_transformer.TransformerEncoder(encoder_layer, num_layers=n_layer, enable_nested_tensor=False)
