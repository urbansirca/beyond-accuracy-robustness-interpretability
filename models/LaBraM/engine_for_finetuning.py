...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    if ch_names is not None:
        input_chans, ch_mask = utils.get_input_chans(ch_names)
    model.train(True)
    ...
    for data_iter_step, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        n_time = t // patch_size
        if n_time > 16:
            samples = samples[:, :, :patch_size*16]

        samples = samples[:,ch_mask,:]
        ...
        mets = utils.get_metrics(torch.sigmoid(output).detach().cpu().numpy(), targets.detach().cpu().numpy(), ["accuracy", "balanced_accuracy"], is_binary)
        class_acc = mets["accuracy"]           
        class_bacc = mets['balanced_accuracy']
            
        metric_logger.update(loss=loss_value)
        metric_logger.update(class_acc=class_acc)
        metric_logger.update(class_bacc=class_bacc)
