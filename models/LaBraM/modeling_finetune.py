...
class Block(nn.Module):
    ...
    def forward(self, x, rel_pos_bias=None, return_attention=False, return_qkv=False):
        ...
        if self.gamma_1 is None:
            x = x + self.drop_path1(self.attn(self.norm1(x), rel_pos_bias=rel_pos_bias))
            x = x + self.drop_path2(self.mlp(self.norm2(x)))
        else:
            x = x + self.drop_path1(self.gamma_1 * self.attn(self.norm1(x), rel_pos_bias=rel_pos_bias))
            x = x + self.drop_path2(self.gamma_2 * self.mlp(self.norm2(x)))
        return x
...
class NeuralTransformer(nn.Module):
    def __init__(self, EEG_size=1600, patch_size=200, in_chans=1, out_chans=8, num_classes=1000, embed_dim=200, depth=12,
        ...
        self.head = nn.Sequential(nn.Dropout(p=0.5),
    ...
    def forward(self, x, input_chans=None, return_patch_tokens=False, return_all_tokens=False, **kwargs):
        ...
        if getattr(self, 'use_concat_pooling', False):
            x = self.forward_features(x, input_chans=input_chans, return_patch_tokens=True, **kwargs)
            x = x.flatten(1)
        else:
