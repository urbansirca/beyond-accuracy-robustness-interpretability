...
def get_input_chans(ch_names):
    """ 
    Modified to also return chmask
    Returns:
        - input_chans = cls token and channel indices 
        - chmask = channel mask in case of channels not present in global list 
    """
    ch_names = np.array(ch_names)
    chmask = np.isin(ch_names, standard_1020)
    ch_names = ch_names[chmask]
    input_chans = [0] # for cls token
    for ch_name in ch_names:
        input_chans.append(standard_1020.index(ch_name.upper()) + 1)
    return input_chans, chmask
