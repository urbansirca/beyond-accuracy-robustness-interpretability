...
class REVE(EEGModuleMixin, nn.Module):
    ...
    **R**\ epresentation for **E**\ EG with **V**\ ersatile **E**\ mbeddings (REVE) from El Ouahidi et al. (2025) [reve]_.
    ...
    def forward(
        ...
        x = self.transformer(x, return_output, exit_block=exit_block)
    ...
    def forward(self, qkv: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_sdpa:  # SDPA Implementation
            with sdpa_kernel(
                ...
                out = F.scaled_dot_product_attention(q, k, v)  # (B, heads, S, head_dim)
        else:  # Naive Implementation
            _, _, scale = q.shape[-2], q.device, q.shape[-1] ** -0.5
            dots = torch.matmul(q, k.transpose(-1, -2)) * scale  # (B, heads, S, head_dim) @ (B, heads, head_dim, S) -> (B, heads, S, S)
            attn = nn.Softmax(dim=-1)(dots)                       # (B, heads, S, S)
            out = torch.matmul(attn, v)                           # (B, heads, S, S) @ (B, heads, S, head_dim) -> (B, heads, S, head_dim)
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        x = self.norm(x)
        qkv = self.to_qkv(x)   # (B, S, dim) @ (dim, 3*heads*head_dim) -> (B, S, 3*heads*head_dim)
        out = self.attend(qkv)  # (B, S, heads*head_dim)
        return self.to_out(out) # (B, S, heads*head_dim) @ (heads*head_dim, dim) -> (B, S, dim)
...
class TransformerBackbone(nn.Module):
    ...
    def forward(
        self, x, return_out_layers=False, exit_block=None,
    ) -> Union[torch.Tensor, list[torch.Tensor]]:
        out_layers = [x] if return_out_layers else []
        layers_to_use = self.layers[:exit_block] if exit_block is not None else self.layers
        for attn, ff in layers_to_use:
            x = attn(x) + x
            x = ff(x) + x
...
class FourierEmb4D(nn.Module):
    ...
    def add_time_patch(cls, pos: torch.Tensor, num_patches: int) -> torch.Tensor:
        ...
        time_values = torch.arange(
            0, num_patches, 1, device=pos.device, dtype=pos.dtype
        )  # Shape: (num_patches,)
